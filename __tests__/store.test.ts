...
describe("Deck Store", () => {
  ...
  it("should update a slide", () => {
    ...
    useDeckStore
      .getState()
      .updateSlide(slideId, { content: { primary: "Updated Content" } });

    const updatedSlide = useDeckStore
      .getState()
      .slides.find((s) => s.id === slideId);
    expect(updatedSlide?.content.primary).toBe("Updated Content");
  });
  ...
  it("should move a slide", () => {
    ...
    expect(slides[0].content.primary).toBe("Slide 2");
    expect(slides[1].content.primary).toBe("Slide 3");
    expect(slides[2].content.primary).toBe("Slide 1");
  });
  ...
  it("should auto-split when platform changes and autoSplit is true", () => {
    ...
    useDeckStore.setState({
      ...
      slides: [{ id: "1", content: { primary: longText }, theme: "linkedin" }],
    });
  ...
  it("should NOT auto-split when autoSplit is false", () => {
    ...
    expect(state.slides[0].content.primary).toBe(longText);
  });
});
